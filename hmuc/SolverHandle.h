...
namespace Minisat {
	...
	public:
		...
		virtual void allocResol(CRef cref, vec<Uid>& allParents, vec<Uid>& icParents);
		virtual void allocNonIcResol(CRef cref);
		virtual void analyzeConflictingAssumptions(Lit initConflict, vec<Lit>& out_negConflicts, vec<uint32_t>& out_icParents, vec<uint32_t>& out_allParents);
		virtual bool inRhombus(Uid uid);
		virtual int level(Var v);

		virtual void updateParentsOrder(Uid uid, const vec<Uid>& icParents, const vec<Uid>& allParents);
		
		virtual void realocExistingResolution(Uid oldUid, const vec<Uid>& icParents, const vec<Uid>& allParents);
